function insertAndStyleText(text) {
    ...
    if (cursor) {
        ...
        const editText = element.editAsText();
        const regex = /(\{.*?\}|\[.*?\]|.*?:)/g;
        let lastIndex = 0;
        let match;
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            // Aplica o estilo padrão ao texto entre as ocorrências correspondentes
            if (lastIndex < start) {
            ...
            // Aplica o estilo personalizado ao texto correspondente
        ...
        // Aplica o estilo padrão ao texto após a última ocorrência correspondente
        if (lastIndex < text.length) {
            editText.setFontFamily(lastIndex, text.length - 1, 'Arial');
            editText.setFontSize(lastIndex, text.length - 1, 11);
        }
    } else {
...
function uvaHTML() {
    const texto =
        ...
        '    O código pode estar em linhas diferentes.\n' +
        '  []\n' +
        '{}\n' +
        ' ';
...
function uvaScrollyAnimado() {
    const texto =
        ...
        '  classe: Classe CSS. Apague se não for utilizar\n' +
        '  guias: sim ou não\n' +
        '  frames: quantidade de frames por segundo\n' +
        '  framesTotais: quantidade de frames totais\n' +
        '    [.+conteúdo]\n' +
        '      {.passo}\n' +
        ...
        '{}\n' +
        ' ';
    const wordsToStyle = ['{.scrolly}', 'tamanho:', 'tamanho_fundo:', 'id:', 'classe:', 'guias:', 'frames:', 'framesTotais:', '[.+conteúdo]', '{.passo}', 'percentual:', 'texto:', '[]', '{}'];
    insertAndStyleText(texto, wordsToStyle);
}
//*******//
